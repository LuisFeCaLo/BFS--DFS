...
public class Generador {
      ...
      public static void main(String[] args) throws IOException {
        ...
        int verificador1=0, verificador2=0, verificador3=0 ,  numeronodos=0, esdirigido=0 , hayauto=0, Nrealizar=1; 
        ...
        HashMap<Integer, Aristas> CNodos = new HashMap<>();
        HashMap<Integer, Aristas> LAristas = new HashMap<>();
        switch ( tipo ) {
        /*------------------------------------------------------------------------------------------------*/      
        case ('A'):
        System.out.println( "Inicio-Erdos" );
       ...
       /* System.out.println("A="+a.Dameidentificador()+"B="+b.Dameidentificador()+"VA="+va.Dameidentificador()+"VB="+vb.Dameidentificador()+"Esdiri="+esdirigido);*/   
        Nrealizar=1;
        ...
        /*System.out.println("Se realiza="+Nrealizar);*/
        if(Nrealizar ==1){
        Aristas arista = new Aristas(a,b,datosarista);
        LAristas.put(contadoraristas,arista);  
        a.Dameadj(b);
       ...
       boolean existe;
        ...
        out.flush();
        a=LNodos.get(0);
        pendientes.add(Integer.toString(a.Dameidentificador()));
        pasados.add(Integer.toString(a.Dameidentificador()));
        while (!pendientes.isEmpty()){
         ...
         d = ot.next();
         b=ANodos.get(d);
            if (pasados.contains(Integer.toString(b.Dameidentificador()))){continue;}
            pendientes.add(Integer.toString(b.Dameidentificador()));
            pasados.add(Integer.toString(b.Dameidentificador()));
            out.println("\"" + a.Dameidentificador() + "\"->\"" + b.Dameidentificador() +  "\"");
            out.flush();
  ...
  /*------------------------------------------------------------------------------------------------------------------------*/
        ...
        LA=DFSR(a.Dameidentificador(),archivo,LNodos);
  ...
  /*------------------------------------------------------------------------------------------------------------------------*/
        ...
        out.flush();
        x=LNodos.get(0);
        pendientes.add(Integer.toString(x.Dameidentificador()));
        pasados.add(Integer.toString(x.Dameidentificador()));
        otros.add(Integer.toString(x.Dameidentificador()));
        while (!otros.isEmpty()){
        while (!pendientes.isEmpty()){
        ...
        d = ot.next();
        y=ANodos.get(d);
        if (pasados.contains(Integer.toString(y.Dameidentificador()))){continue;}
        pendientes.add(Integer.toString(y.Dameidentificador()));
        pasados.add(Integer.toString(y.Dameidentificador()));
        otros.add(Integer.toString(x.Dameidentificador()));
        out.println("\"" + x.Dameidentificador() + "\"->\"" + y.Dameidentificador() +  "\"");
        out.flush();
        break;          }}
     ...
     public static LinkedList<String> DFSR(int q,String archivo,HashMap<Integer, Nodos> LNodos )  {
        ...
        pasados.add(Integer.toString(y.Dameidentificador()));
        otros.add(Integer.toString(x.Dameidentificador()));
        LA.add( "\"" + x.Dameidentificador() + "\"->\"" + y.Dameidentificador() +  "\"");
        break;          }}
        x = LNodos.get(Integer.parseInt(otros.pollLast()));
        ANodos = x.Regresaadj();
        for (Iterator<Integer> ot = ANodos.keySet().iterator(); ot.hasNext();) {
